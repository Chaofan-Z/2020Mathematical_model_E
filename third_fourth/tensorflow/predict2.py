...
def predict():
    ...
    with tf.Session() as sess:
        ...
        for img in imgAll:
            ...
            predImg = pred[0, :, :, 0]
            imgSum = predImg.sum()
            imgSumAll.append(str(imgSum/100))
            # break


        with open('./roadDepth', 'w') as file2:
            for i in imgSumAll:
                file2.write(i + '\n')

        # dateX = [i for i in range(100)]
        # plt.figure(figsize=(15, 7), dpi=150)
        # # plt.xticks(np.arange(100), dateX, rotation=45)
        ...
        #
        # plt.savefig('./roadDepth.jpg', bbox_inches='tight')
        #
        # plt.show()
        # imgSumAll = '\t'.join()
